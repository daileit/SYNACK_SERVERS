...
def detect_cmnd(image):
	...
	for (startX, startY, endX, endY) in boxes:
		...
		if startY < MIN_y:
			MIN_y = startY
		if startX < MIN_X and startX < orig.shape[1]//2 and startX > orig.shape[1]//5:
			MIN_X = startX
		if endX > MAX_X:
			MAX_X = endX
	new_img = orig[MIN_y:MAX_y+30,MIN_X-30:]
	return new_img,MIN_X,MAX_y,MIN_y,MAX_y
def remove_name_line(img):
    ...
    img = cv2.resize(img,(int(img.shape[1]*1.2),int(img.shape[0]*1.2)))
    return img
def crop_image_from_background(img,flag):
	if flag == 'back':
		img1 = cv2.imread('matsau_train.jpg')
	else:
